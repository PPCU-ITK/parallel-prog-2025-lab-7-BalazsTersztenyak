...
void conjugate_gradient_csr(const double* values, const int* col_indices, const int* row_start, const double* b, double* x, int n, int max_iterations, double tolerance) {
    ...
    const int value_size = row_start[n];

    #pragma omp target data map(alloc:r[0:n]) map(alloc:p[0:n]) map(alloc:Ap[0:n]) map(alloc:Ax[0:n]) \
                            map(values[0:value_size]) map(col_indices[0:value_size]) map(row_start[0:n+1]) map(b[0:n]) map(x[0:n])
    {
